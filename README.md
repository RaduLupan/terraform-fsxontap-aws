...
7. (Manual) Configure iSCSI on the FSx ONTAP file system:
    - On the client instance get the initiator name:
    ```$ sudo cat /etc/iscsi/initiatorname.iscsi```
    - Connect to the FSx ONTAP management endpoint from the OPS or client instance:
    ```$ ssh fsxadmin@management_endpoint_ip```
    ...
    - Confirm that the initiator group exists:
    ```$ lun igroup show```
    ...
    - Use the lun show -path command to confirm the LUN is created, online, and mapped:
    ```$ lun show -path /vol/vol_name/lun_name -fields state,mapped,serial-hex```
    example:
    ```$ lun show -path /vol/iscsi_volume2/lun_1 -fields state,mapped,serial-hex```
    - Use the network interface show -vserver command to retrieve the addresses of the iscsi_1 and iscsi_2 interfaces for the SVM in which you've created your iSCSI LUN:
